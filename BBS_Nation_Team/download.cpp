...
bool downloadImage(const string&URL,const string& filename)
{
	...
	std::wstring url = s2ws(URL);
	LPCWSTR Lurl = url.c_str();
	std::wstring file = s2ws(filename);
	LPCWSTR Lfile = file.c_str();
	while(i<retry_attempt&&URLDownloadToFile(NULL,Lurl,Lfile,0,NULL)!=S_OK)
	{
		cout<<"下载图片失败"<<endl;
		cout<<"图片URL: "<<URL<<endl;
		cout<<"重新下载"<<endl;
